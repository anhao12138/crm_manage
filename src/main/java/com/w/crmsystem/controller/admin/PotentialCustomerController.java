...
public class PotentialCustomerController {
    ...
    public String queryAllByKey(Integer page,Integer limit,String keyword,Integer state){
        ...
        if (StringUtils.isEmpty(keyword)&&StringUtils.isEmpty(state)){
            ...
            resultInfo.setData(potentialcustomerSerivce.queryAll(page,limit));
            resultInfo.setCount(potentialcustomerSerivce.countAll());

        }else {
            //条件分页查询

            resultInfo.setData(potentialcustomerSerivce.queryByKey(page,limit,keyword,state));
            resultInfo.setCount(potentialcustomerSerivce.countByKey(keyword,state));
        }
    ...
    public String addAndUpdateInfo(Potentialcustomer potentialcustomer){
        if (StringUtils.isEmpty(potentialcustomer.getAge())){
            new NullInfoException("客户年龄不能为空的哟亲");
        }
        if (StringUtils.isEmpty(potentialcustomer.getJobId())){
            new NullInfoException("客户的年龄咋可能为空呢");
        }
        if (StringUtils.isEmpty(potentialcustomer.getTel())){
            new NullInfoException("客户的电话是不能为空的哟亲");
        }
        ...
        if (StringUtils.isEmpty(potentialcustomer.getPcId())){
            ...
            potentialcustomer.setStatus(0);
            potentialcustomerSerivce.addPotentialInfo(potentialcustomer);
            resultSingleInfo.setMsg("新增"+potentialcustomer.getName()+"客户成功了呢");
        }else {
            //更新操作
            potentialcustomerSerivce.updatePotentialInfo(potentialcustomer);
            resultSingleInfo.setMsg("以将"+potentialcustomer.getName()+"客户的信息更新了哟");
        }
